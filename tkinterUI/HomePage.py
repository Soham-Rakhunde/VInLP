...
class HomePage(Frame):
    def __init__(self, master):
        ...
        self.video = imageio.get_reader(f'{parDir}video.mkv')
        ...
        thread = threading.Thread(target=self.stream, args=(vidLabel,))
        thread.daemon = 1
        thread.start()
    ...
    def stream(self, label):
        for image in self.video.iter_data():
            photoPIL = Image.fromarray(image)
            ...
            label.config(image=frame_image, pady=10)
